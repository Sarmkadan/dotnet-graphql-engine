...
/// Supports an optional namespace prefix for multi-tenant deployments where multiple
/// engine instances share the same cache backend (e.g. Redis).
/// Can be used as a static helper (no prefix) or instantiated with a per-tenant namespace.
/// </summary>
sealed public class CacheKeyBuilder
{
    ...
    /// <summary>
    /// Creates a <see cref="CacheKeyBuilder"/> that prepends <paramref name="ns"/> to every
    /// generated key, isolating cache entries from other tenants or deployments that share
    /// the same cache store.
    /// </summary>
    /// <param name="ns">
    /// Namespace / tenant prefix, e.g. <c>"tenant-a:"</c>.
    /// Pass an empty string (or use the static methods) when no prefix is needed.
    /// </param>
    public CacheKeyBuilder(string ns = "")
    {
        _namespace = ns ?? string.Empty;
    }

    // ------------------------------------------------------------------
    // Instance methods – respect the configured namespace prefix
    // ------------------------------------------------------------------

    /// <summary>Builds a namespaced cache key for a GraphQL query.</summary>
    public string BuildQueryKeyNs(string schemaName, string query, Dictionary<string, object>? variables = null)
        => _namespace + BuildQueryKey(schemaName, query, variables);

    /// <summary>Builds a namespaced cache key for schema metadata.</summary>
    public string BuildSchemaKeyNs(string schemaName)
        => _namespace + BuildSchemaKey(schemaName);

    /// <summary>Builds a namespaced cache key for schema types.</summary>
    public string BuildSchemaTypesKeyNs(string schemaName)
        => _namespace + BuildSchemaTypesKey(schemaName);

    /// <summary>Builds a namespaced cache key for a specific type.</summary>
    public string BuildTypeKeyNs(string schemaName, string typeName)
        => _namespace + BuildTypeKey(schemaName, typeName);

    /// <summary>Builds a namespaced cache key for query execution results.</summary>
    public string BuildExecutionKeyNs(string operationId, string operationName)
        => _namespace + BuildExecutionKey(operationId, operationName);

    /// <summary>Builds a namespaced cache key for metadata.</summary>
    public string BuildMetadataKeyNs(string identifier, string metaType)
        => _namespace + BuildMetadataKey(identifier, metaType);

    /// <summary>Builds a namespaced rate-limit key.</summary>
    public string BuildRateLimitKeyNs(string clientId, string endpoint)
        => _namespace + BuildRateLimitKey(clientId, endpoint);
    ...
    /// <summary>
    /// Creates a <see cref="CacheKeyBuilder"/> pre-configured with the given namespace prefix.
    /// </summary>
    /// <param name="ns">
    /// Tenant or deployment namespace, e.g. <c>"tenant-a:"</c>.
    /// All keys generated by the returned instance will be prefixed with this value.
    /// </param>
    public static CacheKeyBuilder WithNamespace(string ns) => new CacheKeyBuilder(ns);
}
